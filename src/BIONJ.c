...
tree *bionj (double **D, set *species, int n, boolean isNJ)
{
  ...
  /*   initialise and symmetrize the running delta matrix    */
    ...
    *b=0;
	...
	strncat(chain1,":",1);
	snprintf(chain1+strlen(chain1),12,"%lf",la);
	...
	strncpy(chain1,"",1);
	strncat(chain1,":",1);
	snprintf(chain1+strlen(chain1),12,"%lf",lb);
...
void Initialize(double **D, set *species, double **delta, POINTERS *trees, int n)
{
  ...
  do {
    ...
    strncpy (name->name, reader->firstNode->label, MAX_LABEL_LENGTH);
    name->suiv=NULL;
    trees[lig].head=name;
    trees[lig].tail=name;
